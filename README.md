...
- モード変更オプションと対応する値入力オプション
  1. **reso** :画像の解像度を変更する。
  - 値入力オプション:1~100の、画像のクオリティを指定する整数値。
  2. **size**  :画像の縦横の長さを変更する。
  - 値入力オプション:<縦長さ> x <横長さ>を表す2つの整数値を入力する
  3. **get** :画像の縦横の長さとサイズを表示する
  - 値入力オプション:なし
  4. **-util** :このオプションの後に`info`、`debug`のどちらかを入力して使用する
    - *info*: 変更された要素を単位と変更前の値と並べて出力する。
...
# インストール方法
```git clone https://github.com/naiunaiu/fetch.git```
